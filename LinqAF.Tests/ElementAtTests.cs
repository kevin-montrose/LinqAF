...
namespace LinqAF.Tests
{
    ...
    public class ElementAtTests
    {
        ...
        public void InstanceExtensionNoOverlap()
        {
            ...
            Helper.GetOverlappingMethods(typeof(Impl.IElementAt<>), out instOverlaps, out extOverlaps);
        ...
        public void Chaining_Weird()
        {
            ...
            // lookupDefault
            {
                ...
                Assert.IsTrue(lookupDefault.Contains(lookupDefault.First(), new _GroupingComparer<int>()));
            }

            // lookupSpecific
            {
                Assert.IsFalse(lookupSpecific.Contains(lookupDefault.First()));
                Assert.IsTrue(lookupSpecific.Contains(lookupDefault.First(), new _GroupingComparer<int>()));
            }
        ...
        public void Malformed_Weird()
        {
            ...
            // lookupDefault
            {
                try { lookupDefault.ElementAt(1); Assert.Fail(); } catch (ArgumentException exc) { Assert.AreEqual("source", exc.ParamName); }
                try { lookupDefault.ElementAtOrDefault(1); Assert.Fail(); } catch (ArgumentException exc) { Assert.AreEqual("source", exc.ParamName); }
            }

            // lookupSpecific
            {
                try { lookupSpecific.ElementAt(1); Assert.Fail(); } catch (ArgumentException exc) { Assert.AreEqual("source", exc.ParamName); }
                try { lookupSpecific.ElementAtOrDefault(1); Assert.Fail(); } catch (ArgumentException exc) { Assert.AreEqual("source", exc.ParamName); }
            }
