...
namespace LinqAF.Tests
{
    ...
    public class WhereTests
    {
        ...
        public void InstanceExtensionNoOverlap()
        {
            ...
            Helper.GetOverlappingMethods(typeof(Impl.IWhere<,,>), out instOverlaps, out extOverlaps);
        ...
        public void Chaining_Weird()
        {
            ...
            // lookupDefault
            {
                Assert.IsTrue(lookupDefault.Where(x => true).SequenceEqual(groupByDefault, new _GroupingComparer<int>()));
                Assert.IsTrue(lookupDefault.Where((x, ix) => true).SequenceEqual(groupByDefault, new _GroupingComparer<int>()));
            }

            // lookupSpecific
            {
                Assert.IsTrue(lookupSpecific.Where(x => true).SequenceEqual(groupByDefault, new _GroupingComparer<int>()));
                Assert.IsTrue(lookupSpecific.Where((x, ix) => true).SequenceEqual(groupByDefault, new _GroupingComparer<int>()));
            }
        ...
        public void Errors_Weird()
        {
            ...
            // lookupDefault
            {
                try { lookupDefault.Where(default(Func<GroupingEnumerable<int, int>, bool>)); Assert.Fail(); } catch (ArgumentNullException exc) { Assert.AreEqual("predicate", exc.ParamName); }
                try { lookupDefault.Where(default(Func<GroupingEnumerable<int, int>, int, bool>)); Assert.Fail(); } catch (ArgumentNullException exc) { Assert.AreEqual("predicate", exc.ParamName); }
            }

            // lookupSpecific
            {
                try { lookupSpecific.Where(default(Func<GroupingEnumerable<int, int>, bool>)); Assert.Fail(); } catch (ArgumentNullException exc) { Assert.AreEqual("predicate", exc.ParamName); }
                try { lookupSpecific.Where(default(Func<GroupingEnumerable<int, int>, int, bool>)); Assert.Fail(); } catch (ArgumentNullException exc) { Assert.AreEqual("predicate", exc.ParamName); }
            }
        ...
        public void Malformed_Weird()
        {
            ...
            // lookupDefault
            {
                try { lookupDefault.Where(x => true); Assert.Fail(); } catch (ArgumentException exc) { Assert.AreEqual("source", exc.ParamName); }
                try { lookupDefault.Where((x, ix) => true); Assert.Fail(); } catch (ArgumentException exc) { Assert.AreEqual("source", exc.ParamName); }
            }

            // lookupSpecific
            {
                try { lookupSpecific.Where(x => true); Assert.Fail(); } catch (ArgumentException exc) { Assert.AreEqual("source", exc.ParamName); }
                try { lookupSpecific.Where((x, ix) => true); Assert.Fail(); } catch (ArgumentException exc) { Assert.AreEqual("source", exc.ParamName); }
            }
