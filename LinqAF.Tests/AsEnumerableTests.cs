...
namespace LinqAF.Tests
{
    ...
    public class AsEnumerableTests
    {
        ...
        public void InstanceExtensionNoOverlap()
        {
            ...
            Helper.GetOverlappingMethods(typeof(Impl.IAsEnumerable<>), out instOverlaps, out extOverlaps);
        ...
        public void Universal()
        {
            ...
            foreach (var e in enums)
            {
                ...
                if (!Helper.Implements(e, typeof(Impl.IAsEnumerable<>), out missing))
                {
                    Assert.Fail($"{e.Name} does not implement IAsEnumerable ({string.Join(", ", missing)})");
                }
            }
        }
        ...
        public void Chaining_Weird()
        {
            ...
            // lookupDefault
            {
                Assert.IsTrue(lookupDefault.SequenceEqual(lookupDefault.AsEnumerable(), new _GroupingComparer<int>()));
            }

            // lookupSpecific
            {
                Assert.IsTrue(lookupSpecific.SequenceEqual(lookupDefault.AsEnumerable(), new _GroupingComparer<int>()));
            }
        ...
        public void Malformed_Weird()
        {
            ...
            // lookupDefault
            {
                try { lookupDefault.AsEnumerable(); Assert.Fail(); } catch (ArgumentException exc) { Assert.AreEqual("source", exc.ParamName); }
            }

            // lookupSpecific
            {
                try { lookupSpecific.AsEnumerable(); Assert.Fail(); } catch (ArgumentException exc) { Assert.AreEqual("source", exc.ParamName); }
            }
