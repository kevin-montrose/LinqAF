...
namespace LinqAF.Tests
{
    ...
    public class SelectTests
    {
        ...
        public void InstanceExtensionNoOverlap()
        {
            ...
            Helper.GetOverlappingMethods(typeof(Impl.ISelect<,,>), out instOverlaps, out extOverlaps);
        ...
        public void Chaining_Weird()
        {
            ...
            // lookupDefault
            {
                Assert.IsTrue(lookupDefault.Select(x => x).SequenceEqual(groupByDefault, new _GroupingComparer<int>()));
                Assert.IsTrue(lookupDefault.Select((x, ix) => x).SequenceEqual(groupByDefault, new _GroupingComparer<int>()));
            }

            // lookupSpecific
            {
                Assert.IsTrue(lookupSpecific.Select(x => x).SequenceEqual(groupByDefault, new _GroupingComparer<int>()));
                Assert.IsTrue(lookupSpecific.Select((x, ix) => x).SequenceEqual(groupByDefault, new _GroupingComparer<int>()));
            }
        ...
        public void Errors_Weird()
        {
            ...
            // lookupDefault
            {
                try { lookupDefault.Select(default(Func<GroupingEnumerable<int, int>, int>)); Assert.Fail(); } catch (ArgumentNullException exc) { Assert.AreEqual("selector", exc.ParamName); }
                try { lookupDefault.Select(default(Func<GroupingEnumerable<int, int>, int, int>)); Assert.Fail(); } catch (ArgumentNullException exc) { Assert.AreEqual("selector", exc.ParamName); }
            }

            // lookupSpecific
            {
                try { lookupSpecific.Select(default(Func<GroupingEnumerable<int, int>, int>)); Assert.Fail(); } catch (ArgumentNullException exc) { Assert.AreEqual("selector", exc.ParamName); }
                try { lookupSpecific.Select(default(Func<GroupingEnumerable<int, int>, int, int>)); Assert.Fail(); } catch (ArgumentNullException exc) { Assert.AreEqual("selector", exc.ParamName); }
            }
        ...
        public void Malformed_Weird()
        {
            ...
            // lookupDefault
            {
                try { lookupDefault.Select(x => x); Assert.Fail(); } catch (ArgumentException exc) { Assert.AreEqual("source", exc.ParamName); }
                try { lookupDefault.Select((x, ix) => x); Assert.Fail(); } catch (ArgumentException exc) { Assert.AreEqual("source", exc.ParamName); }
            }

            // lookupSpecific
            {
                try { lookupSpecific.Select(x => x); Assert.Fail(); } catch (ArgumentException exc) { Assert.AreEqual("source", exc.ParamName); }
                try { lookupSpecific.Select((x, ix) => x); Assert.Fail(); } catch (ArgumentException exc) { Assert.AreEqual("source", exc.ParamName); }
            }
